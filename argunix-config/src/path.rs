...
mod tests {
    ...
    fn expands_dollar_var() {
        with_env(
            "ARGUNIX_TEST_DOLLAR",
            "/run/credentials/argunix.service",
            || {
                assert_eq!(
                    resolve_path("$ARGUNIX_TEST_DOLLAR/token").unwrap(),
                    PathBuf::from("/run/credentials/argunix.service/token"),
                );
            },
        );
    }
    ...
    fn expands_braced_var() {
        with_env("ARGUNIX_TEST_BRACED", "abc", || {
            assert_eq!(
                resolve_path("/x/${ARGUNIX_TEST_BRACED}/y").unwrap(),
    ...
    fn errors_on_undefined_var() {
        ...
        assert!(matches!(
            resolve_path("/a/$ARGUNIX_DEFINITELY_NOT_SET_xyzzy/b"),
