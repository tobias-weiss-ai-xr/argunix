...
/// Constrained so it works as a filename component (`/run/medusa/builders/<name>.sock`),
/// a column in `medusactl builders` output, and a URL-safe segment.
...
/// reconnect. Source-of-truth lives on the builder; medusa caches the latest
...
mod tests {
    ...
    fn builder_name_rejects_slash() {
        // we route via /run/medusa/builders/<name>.sock — slashes break that.
