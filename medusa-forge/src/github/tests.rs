...
async fn post_check_succeeds() {
    ...
    Mock::given(method("POST"))
        ...
        .and(body_json(serde_json::json!({
            "state": "pending",
            "context": "medusa: evaluating",
            "description": "kicking off eval",
            "target_url": "https://medusa.example/r/github/myorg/myrepo/eval/1"
        })))
    ...
    let handle = provider
        .post_check(CheckPost {
            ...
            context: "medusa: evaluating".to_string(),
            state: CheckState::Pending,
            description: Some("kicking off eval".to_string()),
            target_url: Some("https://medusa.example/r/github/myorg/myrepo/eval/1".to_string()),
        })
        .await
        .unwrap();
