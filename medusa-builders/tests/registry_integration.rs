...
where
    F: FnMut(&medusa_builders::BuilderSnapshot) -> bool,
{
...
async fn duplicate_name_displaces_old_connection() {
    ...
    // Two distinct keys: medusa upserts the row by name on hello.
    ...
    // medusa upserts the row with pubkey_b and the registry entry is
    // displaced.
