//! argunix-builder agent entry point.
//!
...
use argunix_builder_agent::{AgentConfig, discover_capabilities, load_or_generate, run};
...
#[derive(Parser, Debug)]
#[command(
    name = "argunix-builder",
    about = "Dial argunix, register as a builder, serve nix-store builds"
)]
struct Cli {
    /// Hostname or IP of the argunix daemon.
    #[arg(long)]
    argunix_host: String,
    /// Builder-enrollment SSH port on argunix (default 2222).
    #[arg(long, default_value_t = 2222)]
    argunix_port: u16,
    /// Path to the file containing the shared enrollment token.
    /// Used only on first connect or after `argunixctl builders revoke`.
    ...
    #[arg(long, default_value = "/var/lib/argunix-builder")]
    state_dir: PathBuf,
...
async fn main() -> Result<()> {
    ...
    let name_str = cli
        ...
        .unwrap_or_else(|| "argunix-builder".into());
    ...
    let host_port = format!("{}:{}", cli.argunix_host, cli.argunix_port);
    let argunix_addr = lookup_host(&host_port)
        .await
        .with_context(|| format!("resolving argunix address `{host_port}`"))?
        .next()
    ...
    let cfg = AgentConfig {
        ...
        // Pin argunix's SSH host key under the agent's state dir so a
...
/// newline; argunix's server side compares the exact bytes.
