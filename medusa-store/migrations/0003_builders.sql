-- Dynamic builder pool (M13). Builders dial medusa over SSH (russh server)
-- and self-describe their capabilities; medusa never declares them.
--
-- Auth model is TOFU:
--   * first connect uses a shared enrollment token (password method),
--     after which medusa stores `(name, pubkey, capabilities)` here;
