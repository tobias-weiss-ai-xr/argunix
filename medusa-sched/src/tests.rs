...
fn single_repo_dispatches_in_fifo_order() {
    let mut s = Scheduler::new(None);
    s.enqueue(rid(1), jid(10));
    s.enqueue(rid(1), jid(11));
    s.enqueue(rid(1), jid(12));
...
fn weight_2_to_1_dispatches_at_2_to_1_ratio() {
    ...
    for n in 0..1000 {
        s.enqueue(rid(1), jid(n));
        s.enqueue(rid(2), jid(1000 + n));
    }
...
fn concurrency_cap_blocks_dispatch_until_complete() {
    let mut s = Scheduler::new(Some(2));
    s.enqueue(rid(1), jid(1));
    s.enqueue(rid(1), jid(2));
    s.enqueue(rid(1), jid(3));
...
fn complete_returns_repo_for_known_jobs_and_none_otherwise() {
    let mut s = Scheduler::new(None);
    s.enqueue(rid(7), jid(100));
    let d = s.dispatch().unwrap();
    assert_eq!(s.complete(d.job_id), Some(rid(7)));
    assert_eq!(s.complete(d.job_id), None);
    assert_eq!(s.complete(jid(999_999)), None);
}
...
fn many_repos_random_arrivals_no_starvation() {
    ...
    // empties.
    let mut s = Scheduler::new(Some(3));
    for i in 1..=5 {
        s.set_weight(rid(i), (i as u32 - 1).max(1));
    }
    for round in 0..500 {
        for repo in 1..=5_i64 {
            s.enqueue(rid(repo), jid(round * 5 + repo));
        }
    }
...
fn newly_arriving_repo_is_not_starved_by_a_busy_high_weight_one() {
    ...
    s.enqueue(rid(2), jid(1000));
    s.enqueue(rid(2), jid(1001));
    s.enqueue(rid(2), jid(1002));
