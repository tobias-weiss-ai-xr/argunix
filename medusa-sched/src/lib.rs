...
use medusa_domain::{JobId, RepoId};
...
impl Scheduler {
    ...
    pub fn set_weight(&mut self, repo_id: RepoId, weight: u32) {
        let state = self
            .repos
            .entry(repo_id)
    ...
    pub fn enqueue(&mut self, repo_id: RepoId, job_id: JobId) {
    ...
    /// No-op kept for callers that drive the scheduler with a periodic
    /// tick. The current algorithm is virtual-time-based, so wall-clock
    ...
    pub fn complete(&mut self, job_id: JobId) -> Option<RepoId> {
    ...
    pub fn pending_for(&self, repo_id: RepoId) -> usize {
        self.repos
            .get(&repo_id)
            .map(|s| s.pending.len())
            .unwrap_or(0)
