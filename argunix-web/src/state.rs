...
use argunix_domain::{EvalId, ForgeKind};
use argunix_forge::{ForgejoProvider, GithubProvider, GitlabProvider, Provider};
...
pub struct AppStateInner {
    ...
    /// `argunixctl reload`. Read with `current.load_full()` to snapshot.
...
pub enum BuildProvidersError {
    ...
    Auth {
        forge: String,
        #[source]
        error: argunix_config::ForgeAuthShapeError,
    },
