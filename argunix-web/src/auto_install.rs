...
//! argunix with a matching secret. Idempotent — runs every boot, no-ops
//! when the forge already agrees with sqlite.
//!
//! Best-effort: a forge being unreachable doesn't block daemon startup.
//! We log warnings per repo and the rest of argunix keeps working;
//! webhooks that didn't get installed will return
//! `WebhookNotProvisioned` (HTTP 503) when (if) they fire.

use argunix_config::{Config, Repo};
use argunix_domain::ForgeKind;
use argunix_forge::Provider;
use argunix_store::{RepoStore, SqlxStore};
use rand::RngCore;
...
async fn ensure_one(
    ...
    // argunix's verify side to use the same key bytes, we must keep
