...
use argunix_builders::{BuilderRegistry, BuilderServer, ServerConfig, load_or_generate};
...
async fn token_auth_rejects_wrong_token() {
    ...
    let mut client = make_client(addr).await;
    let result = client
        .authenticate_password("argunix-builder", "bogus-token")
        .await
        .unwrap();
...
async fn pubkey_auth_rejects_revoked_row() {
    ...
    // fresh token after `argunixctl builders revoke`).
    let _ = <SqlxStore as BuilderStore>::upsert(
        &store,
        NewBuilder {
