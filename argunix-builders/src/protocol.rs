...
pub enum ControlMessage {
    /// Builder → argunix. Sent immediately after auth completes; carries
    ...
    /// Argunix → builder. Sent in reply to Hello. `builder_id` is the
    ...
    /// Argunix → builder. Sent on revoke or duplicate-name takeover.
    ...
    /// Builder → argunix. Acknowledges that the agent received a
    ...
    /// Builder → argunix. A chunk of raw stderr bytes from
    ...
    /// Builder → argunix. Terminal status of one dispatched build.
    ...
    /// Argunix → builder. Cancel a running build. The agent SIGKILLs
...
mod tests {
    ...
    fn build_round_trip() {
        let m = ControlMessage::Build {
            ...
            gc_root: Some("/nix/var/nix/gcroots/per-user/argunix/1/2/3".into()),
            timeout_secs: 7200,
